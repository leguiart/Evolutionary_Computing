...
class NoveltyEvaluator(IEvaluator):
    ...
    def evaluate(self, X):
        ...
        for i in range(len(X)):
            X[i].fitness_metric = self._average_knn_distance(X[i], X_copy)
            if(X[i].fitness_metric > self.novelty_threshold or len(self.novelty_archive) < self.min_novelty_archive_size):
                self.items_added_in_generation+=1
                self.novelty_archive += [X[i]]
