...
class _BaseBinaryGAProblem(_BaseGAProblem):
    ...
    def _decode(self, X):
        ...
        decoded_rep = np.zeros((len(X_mat), self.n_dim))
        for i in range(self.n_dim):
            binary_powers = 2**np.arange(X_mat[:, i*self.gene_length : (i + 1)*self.gene_length].shape[1], dtype = np.float64)[::-1][:, np.newaxis]
            decoded_variable = (X_mat[:, i*self.gene_length : (i + 1)*self.gene_length]@binary_powers).T
            decoded_rep[:,i] = decoded_variable
        decoded_rep = self.bounds[0] + decoded_rep*(self.bounds[1] - self.bounds[0])/(2**self.gene_length - 1)
        for i in range(len(X)):
            X[i].phenotype = decoded_rep[i, :].copy()
        return X
    ...
    def _crossover(self, X, pc, elitism_num):
        ...
        for i, p in enumerate(prob_cross):
            if p <= pc:
                ...
                son1 = X_mat[2*i,:].copy()
                son2 = X_mat[2*i + 1, :].copy()
                son1[cross_point + 1 : X_mat.shape[1]] = X_mat[2*i + 1, cross_point + 1: X_mat.shape[1]].copy()
                son2[0 : cross_point + 1] = X_mat[2*i, 0 : cross_point + 1].copy()
                X_mat[2*i,:] = son1
                X_mat[2*i + 1,:] = son2
